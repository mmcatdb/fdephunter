...
import de.metanome.algorithms.hyfd.structures.PLIBuilderModified;
...
public class HyFDModified implements FunctionalDependencyAlgorithm, BooleanParameterAlgorithm, IntegerParameterAlgorithm, RelationalInputParameterAlgorithm {
...
//    private RelationalInputGenerator inputGenerator = null;
    ...
    private String[] ourHeader;
    private List<String[]> ourData;
    ...
    public void setOurData(String[] header, List<String[]> data) {
        this.ourHeader = header;
        this.ourData = data;
    }

    @Override
    public void setBooleanConfigurationValue(String identifier, Boolean... values) throws AlgorithmConfigurationException {
        if (HyFDModified.Identifier.NULL_EQUALS_NULL.name().equals(identifier)) {
            this.valueComparator = new ValueComparator(values[0].booleanValue());
        } else if (HyFDModified.Identifier.VALIDATE_PARALLEL.name().equals(identifier)) {
            this.validateParallel = values[0].booleanValue();
        } else if (HyFDModified.Identifier.ENABLE_MEMORY_GUARDIAN.name().equals(identifier)) {
...
//        }
    ...
    private void initialize(/*RelationalInput relationalInput*/) throws AlgorithmExecutionException {
        this.tableName = "MODIFIED_TABLE_NAME_TODO";//relationalInput.relationName();
        this.numAttributes = ourHeader.length;
...
//        RelationalInput relationalInput = this.getInput();
        ...
        PLIBuilderModified pliBuilder = new PLIBuilderModified(this.inputRowLimit);
        List<PositionListIndex> plis = pliBuilder.getPLIs(ourData, this.numAttributes, this.valueComparator.isNullEqualNull());
//        this.closeInput(relationalInput);
    ...
    private ObjectArrayList<ColumnIdentifier> buildColumnIdentifiers() {
        ObjectArrayList<ColumnIdentifier> columnIdentifiers = new ObjectArrayList<ColumnIdentifier>(ourHeader.length);
        for (String attributeName : ourHeader) {
...
//        configs.add(new ConfigurationRequirementRelationalInput(HyFDModified.Identifier.INPUT_GENERATOR.name()));
//
//        ConfigurationRequirementBoolean nullEqualsNull = new ConfigurationRequirementBoolean(HyFDModified.Identifier.NULL_EQUALS_NULL.name());
...
//
//        ConfigurationRequirementBoolean validateParallel = new ConfigurationRequirementBoolean(HyFDModified.Identifier.VALIDATE_PARALLEL.name());
...
//
//        ConfigurationRequirementBoolean enableMemoryGuardian = new ConfigurationRequirementBoolean(HyFDModified.Identifier.ENABLE_MEMORY_GUARDIAN.name());
...
//
//        ConfigurationRequirementInteger maxLhsSize = new ConfigurationRequirementInteger(HyFDModified.Identifier.MAX_DETERMINANT_SIZE.name());
...
//
//        ConfigurationRequirementInteger inputRowLimit = new ConfigurationRequirementInteger(HyFDModified.Identifier.INPUT_ROW_LIMIT.name());
...
//    @Override
//    public void setIntegerConfigurationValue(String identifier, Integer... values) throws AlgorithmConfigurationException {
//        if (HyFDModified.Identifier.MAX_DETERMINANT_SIZE.name().equals(identifier)) {
//            this.maxLhsSize = values[0].intValue();
//        } else if (HyFDModified.Identifier.INPUT_ROW_LIMIT.name().equals(identifier)) {
//            if (values.length > 0) {
...
////    @Override
////    public void setRelationalInputConfigurationValue(String identifier, RelationalInputGenerator... values) throws AlgorithmConfigurationException {
//////        if (HyFDModified.Identifier.INPUT_GENERATOR.name().equals(identifier)) {
...
//        ObjectArrayList<List<String>> records = this.loadData(ourData);
...
//    
//
//    private ObjectArrayList<List<String>> loadData(List<String[]> data) throws InputIterationException {

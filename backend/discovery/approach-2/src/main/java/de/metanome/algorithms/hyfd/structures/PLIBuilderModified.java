...
public class PLIBuilderModified {
    ...
    public PLIBuilderModified(int inputRowLimit) {
        this.inputRowLimit = inputRowLimit;
    }

    public List<PositionListIndex> getPLIs(List<String[]> relationalInput, int numAttributes, boolean isNullEqualNull) throws InputIterationException {
    ...
    protected List<HashMap<String, IntArrayList>> calculateClusterMaps(List<String[]> relationalInput, int numAttributes) throws InputIterationException {
        ...
        this.numRecords = 0;
        int index = 0;
        while (index < relationalInput.size() && (this.inputRowLimit <= 0 || this.inputRowLimit != this.numRecords)) {
            List<String> record = List.of(relationalInput.get(index));
            ++index;
